...
Dark Theme with Financial Focus + ML Predictions
"""
...
def predict_single(model, features):
    ...
    # Get feature names
    try:
        with open(os.path.join(os.path.dirname(__file__), '..', 'models', 'feature_names.txt'), 'r') as f:
...
purposes = ['All'] + sorted(loans['loan_purpose'].dropna().unique().tolist())
selected_purposes = st.sidebar.multiselect("Loan Purpose", options=purposes, default=['All'])
if 'All' in selected_purposes or not selected_purposes:
    selected_purposes = loans['loan_purpose'].dropna().unique().tolist()

homeownerships = ['All'] + sorted(loans['homeownership'].dropna().unique().tolist())
selected_homeownerships = st.sidebar.multiselect("Homeownership", options=homeownerships, default=['All'])
if 'All' in selected_homeownerships or not selected_homeownerships:
    selected_homeownerships = loans['homeownership'].dropna().unique().tolist()

filtered_loans = loans[
    (loans['grade'].isin(selected_grades)) &
    (loans['state'].isin(selected_states)) &
    (loans['loan_purpose'].isin(selected_purposes)) &
    (loans['homeownership'].isin(selected_homeownerships))
]
...
with tab1:
    ...
    with col1:
        st.markdown('<div class="card"><h3>Loan Status Distribution</h3>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="card"><h3>Loans by Grade</h3>', unsafe_allow_html=True)
    ...
    with col3:
        st.markdown('<div class="card"><h3>Loans by Purpose</h3>', unsafe_allow_html=True)
        purpose_counts = filtered_loans['loan_purpose'].value_counts().head(10).reset_index()
    ...
    with col4:
        st.markdown('<div class="card"><h3>Loans by Homeownership</h3>', unsafe_allow_html=True)
        home_counts = filtered_loans['homeownership'].value_counts().reset_index()
...
with tab2:
    if not risk.empty:
        ...
        with col1:
            st.markdown('<div class="card"><h3>Default Rate by Grade</h3>', unsafe_allow_html=True)
            fig = px.bar(
                risk,
                x='grade',
                y='default_rate',
        ...
        with col2:
            st.markdown('<div class="card"><h3>Risk Category Distribution</h3>', unsafe_allow_html=True)
        ...
        with col3:
            st.markdown('<div class="card"><h3>Average DTI by Grade</h3>', unsafe_allow_html=True)
            fig = px.bar(
                risk,
                x='grade',
                y='avg_dti',
        ...
        with col4:
            st.markdown('<div class="card"><h3>Default Rate by Purpose</h3>', unsafe_allow_html=True)
            purpose_risk = filtered_loans.groupby('loan_purpose').apply(
...
with tab3:
    if not portfolio.empty:
        ...
        with col1:
            st.markdown('<div class="card"><h3>Portfolio Summary</h3>', unsafe_allow_html=True)
            st.dataframe(
        ...
        with col2:
            st.markdown('<div class="card"><h3>Performance Metrics</h3>', unsafe_allow_html=True)
            metrics = {
...
with tab5:
    ...
    st.markdown(f'<p style="color: #64748b; font-size: 0.9rem;">Showing {len(filtered_loans):,} of {len(loans):,} total records</p>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

# ===== Footer =====
st.markdown(f"""
<div class="footer">
    <p>🏦 Financial Loan Performance Analytics • Built with Streamlit • Data updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}</p>
</div>
""", unsafe_allow_html=True)